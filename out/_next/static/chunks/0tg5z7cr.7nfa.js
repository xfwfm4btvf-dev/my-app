...
Start today: run npm audit, review your lock file, and enable provenance on packages you maintain. Security is not a feature, it is a discipline.`},{slug:"llm-inference-optimization-simd-2026",title:"SIMD-Aware LLM Inference: Beyond the Low-Hanging Fruit",excerpt:"How SIMD-level optimizations are pushing LLM inference throughput past what naive implementations achieve, and why it matters for local-first AI.",date:"2026-05-12",tags:["AI","LLM","Performance","Systems","Optimization"],content:`# SIMD-Aware LLM Inference: Beyond the Low-Hanging Fruit
...
The next frontier is runtime SIMD dispatch — detecting CPU capabilities at startup and selecting the optimal kernel dynamically. Combined with speculative decoding and continuous batching, local inference is approaching GPU-level throughput on modern CPUs.`}];e.s(["getAllTags",0,function(){let e=new Set;return t.forEach(t=>t.tags.forEach(t=>e.add(t))),Array.from(e)},"getPostsByTag",0,function(e){return t.filter(t=>t.tags.includes(e))},"posts",0,t])}]);